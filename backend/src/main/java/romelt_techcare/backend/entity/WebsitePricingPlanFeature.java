...
/**
 * ================================================================
 * ROMELT TECHCARE — WEBSITE PRICING PLAN FEATURE ENTITY
 * ================================================================
 *
 * Purpose:
 * Stores one customer-facing feature belonging to a specific
 * WebsitePricingPlanVersion.
 *
 * Responsibilities:
 * - Associates a feature with one pricing-plan version.
 * - Stores the public feature text.
 * - Stores an optional frontend-recognized icon key.
 * - Controls display order and active status.
 * - Records administrator attribution.
 * - Supports optimistic locking.
 *
 * Version ownership:
 * Features belong to WebsitePricingPlanVersion, not directly to
 * WebsitePricingPlan.
 *
 * Mutation rules:
 ...
 *
 * Features belonging to PUBLISHED or ARCHIVED versions are preserved
 * as immutable historical content.
 * ================================================================
 */
@Entity
@Table(
        name = "website_pricing_plan_features",
        indexes = {
                @Index(
                        name = "idx_website_pricing_features_order",
                        columnList =
                                "pricing_plan_version_id, is_active, display_order"
                )
        }
)
@Getter
...
@NoArgsConstructor
@AllArgsConstructor
public class WebsitePricingPlanFeature {

    @Id
    @GeneratedValue(strategy = GenerationType.UUID)
    @Column(
            name = "pricing_plan_feature_id",
            nullable = false,
            updatable = false
    )
    private UUID pricingPlanFeatureId;
